...
### Issue lifecycle
- **D-04:** **Rolling open issue, auto-close on green** — mirror the dedupe pattern in `.github/workflows/upstream-drift.yml:170-194` (search open issues by stable title prefix; comment on existing or create new with label). On a subsequent **passing** scheduled run, the workflow auto-closes **every** open issue matching the same title prefix and `docs-reliability` label (not just the first) and posts a single "resolved by run #X" comment on each before closing — defensive against any historical multiple-issue state from a maintainer manually opening a duplicate or from prior dedupe races.
...
- **D-11:** Body is generated by a small inline `node` snippet (or inline shell + `jq`) that reads `docs-smoke-report.json` and emits the markdown — kept inside the workflow step to avoid adding new files under `scripts/reliability/` for a 30-line transform. Three report-state fallbacks for the body renderer, in priority order:
  1. **Report missing entirely** (catastrophic pre-smoke failure — e.g., `bun install`, Playwright install, or process crashed before smoke ran): minimal body — run link + "smoke runner did not produce a report — see run logs."
  2. **Report present but malformed/unparseable**: minimal body — run link + "smoke report could not be parsed — see run logs and uploaded artifact" + link to the report artifact.
  3. **Report parses and shows zero required failures but the job nonetheless failed** (e.g., a post-smoke step crashed): minimal body — run link + "job failed after smoke checks passed — see run logs." Do NOT advertise smoke as failed when it wasn't.

  Only the all-clear path (report parses + reports ≥1 required failure) produces the full structured-summary body from D-10.
